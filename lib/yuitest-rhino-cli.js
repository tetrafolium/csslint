...
YUITest.Object = {
  ...
  _isObject : function(o, failfn) {
    var t = typeof o;
    return (o && (t === 'object' || (!failfn && (t === 'function' ||
                                                 typeof t === 'function')))) ||
           false;
  },
   ...
   *
   * Note that keys are returned in enumeration order (that is, in the same
   * order that they would be enumerated by a `for-in` loop), which may not be
   * the same as the order in which they were defined.
   *
  ...
  keys : Object.keys ||
             function(obj) {
               ...
               if (YUITest.Object._hasEnumBug) {
                 for (i = 0, len = YUITest.Object._forceEnum.length; i < len;
                      ++i) {
...
YUITest.ObjectAssert = {
  ...
  areEqual : function(expected, actual, message) {
    ...
    if (expectedKeys.length != actualKeys.length) {
      YUITest.Assert.fail(YUITest.Assert._formatMessage(
          message, "Object should have " + expectedKeys.length +
                       " keys but has " + actualKeys.length));
    }
...
YUITest.CLI = {
  ...
  print : function(message) { System.out.print(message); },

  println : print,

  warn : function(message) { System.err.print(message); },
  ...
  getFullPath : function(
      filename) { return (new File(filename)).getCanonicalPath(); },

  readFile : readFile,

  processFiles : function() {
    var files = this.files, i, len, output;
...
YUITest.CLI.Logger = function() {
  var testRunner = YUITest.TestRunner, cli = YUITest.CLI;
  ...
  function handleEvent(event) {
    ...
    case testRunner.COMPLETE_EVENT:
      message = "Testing completed at " + (new Date()).toString() + ".\n" +
                ...
                " Total:" + event.results.total + "(" + event.results.ignored +
                " ignored)";
      messageType = "info";
      break;
...
YUITest.CLI.XUnit = function() {
  var testRunner = YUITest.TestRunner, cli = YUITest.CLI, errors = [],
      failures = [], stack = [];
  ...
  function handleEvent(event) {
    ...
    var message = "", results = event.results, i, len;
...
YUITest.Util.mix(YUITest.CLI, {
  ...
  options :
      {verbose : false, webcompat : false, help : false, format : "xunit"},

  outputHelp : function() {
    this.print([
      "\nUsage: yuitest [options] [file|dir]*", " ", "Global Options",
  ...
  processArguments : function() {
    var args = this.args, arg = args.shift(), files = [];
  ...
  start : function() {
    ...
    YUITest.TestRunner.subscribe(
        ...
        function(event) { YUITest.CLI.quit(event.results.failed ? 1 : 0); });
