...
YUITest.Mock.expect = function(mock /*:Object*/, expectation /*:Object*/) {
    ...
    if (expectation.method) {
        var name = expectation.method,
            ...
            callCount =
                typeof expectation.callCount == "number"
            ...
            function serializeToTAP(results) {
                ...
                switch (results.type) {
                    case "test":
                        if (results.result != "ignore") {
                            text =
                                "ok " + currentTestNum++ + " - " + results.name;
    ...
    object = {
        ...
        simulate: function(target, type, options) {
            ...
            if (mouseEvents[type]) {
                ...
                throw new Error(
                    "simulate(): Event '" + type + "' can't be simulated."
                );
            }
        }
    };
...
YUITest.Reporter.prototype = {
    ...
    report: function(results) {
        ...
        for (var prop in this._fields) {
            ...
            if (
                this._fields.hasOwnProperty(prop) &&
        ...
        _buildTestTree: function() {
            ...
            for (var i = 0; i < this.masterSuite.items.length; i++) {
                if (this.masterSuite.items[i] instanceof YUITest.TestSuite) {
                    this._addTestSuiteToTestTree(
                        this._root,
                        this.masterSuite.items[i]
                    );
                } else if (
                    this.masterSuite.items[i] instanceof YUITest.TestCase
                ) {
                    this._addTestCaseToTestTree(
                        this._root,
                        this.masterSuite.items[i]
                    );
                }
                        ...
                        if (typeof thrown.delay == "number") {
                            ...
                            if (typeof setTimeout != "undefined") {
                                testCase.__yui_wait = setTimeout(function() {
                                    YUITest.TestRunner._resumeTest(
                                        thrown.segment
                                    );
                                }, thrown.delay);
                                this._waiting = true;
