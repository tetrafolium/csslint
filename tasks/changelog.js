...
module.exports = function(grunt) {
  grunt.registerMultiTask("changelog", "Write the changelog file", function() {
    ...
    grunt.util.spawn({cmd : "git", args : [ "tag" ]}, function(error, result) {
      // Find the latest git tag
      var tags = result.stdout.split("\n"),
          semver = tags[0].replace("v", "").split("."),
          major = parseInt(semver[0], 10), minor = parseInt(semver[1], 10),
      ...
      grunt.util.spawn(
          ...
          function(error, result) {
            ...
            var template =
                "<%= grunt.template.today('mmmm d, yyyy') %> - v<%= pkg.version %>\n\n" +
