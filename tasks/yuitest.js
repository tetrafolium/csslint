...
module.exports = function(grunt) {
  grunt.registerMultiTask(
      "yuitest", "Run the YUITests for the project", function() {
        var YUITest = require("yuitest");
        var CSSLint =
            require("../dist/csslint-node").CSSLint; // jshint ignore:line
        var files = this.filesSrc;
        var TestRunner = YUITest.TestRunner;
        var done = this.async();
        var failures = [], stack = [];

        // Eval each file so the tests are brought into this scope where CSSLint
        // and YUITest are loaded already
        files.forEach(function(filepath) { eval(grunt.file.read(filepath)); });

        // From YUITest Node CLI with minor colourization changes
        function handleEvent(event) {

          var message = "", results = event.results, i, len, gruntFailMessage;
          ...
          case TestRunner.BEGIN_EVENT:
            ...
            if (TestRunner._groups) {
              grunt.verbose.writeln("Filtering on groups '" +
